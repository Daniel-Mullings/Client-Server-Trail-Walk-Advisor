...
'''Define function called "echoServer": Implements client/server communication
   ...
   Output: Generate/Respond to client request
...
'''Define function called "queryResponse": Implements generating response to client query
   Input:  "p_query" = Query from client
...
def queryResponse(p_query):
    ...
    return "Invalid Query\n"                                                #Return string literal to subsequent caller of "queryResponse()"
                                                                            ...
                                                                            #      "queryResponse()" return values returned/printed as server response to client query
...
def querySearch(p_query):
    queryAsComponents = p_query.split()                                     #Store whitespace separated words in "p_query" in "queryAsComponent"
    queryAsComponents.pop(0)                                                #Remove '0'-th element in "queryAsComponent", represents query action
    ...
    #Populate "searchRequests" with lists containing search parameters for search requests provided by "queryAsComponents"
    #NOTE: A single search request takes four consecutive elements from "queryAsComponents" to construct a single search request
    #      Reference "Query Builder" at top of "server.py"
    for index in range(0, len(queryAsComponents), 4):
        searchRequests.append(queryAsComponents[index:index+4])
...
def queryBuy(p_query):
    queryAsComponents = p_query.split()                                     #Store whitespace separated words in "p_query" in "queryAsComponent"
    queryAsComponents.pop(0)                                                #Remove '0'-th element in "queryAsComponent", which represents query action
    
    customerName = str(queryAsComponents[0]).capitalize()                   #Set "customerName" to '0'-th element in "queryAsComponents" 
                                                                            #(Customer name becomes '0'-th' element in "queryAsComponents" after "pop()" initial  '0'-th element defining query action
    queryAsComponents.pop(0)                                                #Remove '0'-th' element in "queryAsComponent", which represents customer name
                                                                            #(Remaining elements represent structure for buy requests)
    ...
    #Populate "buyRequests" with lists containing buy parameters for buy requests provided in "queryAsComponents"
    #NOTE: A single buy request takes two consecutive elements from "queryAsComponents" to construct a single buy request
    #NOTE: Reference "Query Builder" at top of "server.py"
    for index in range(0, len(queryAsComponents), 2):
        buyRequests.append(queryAsComponents[index:index+2])
    ...
    if len(booksOrdered) > 0:                                               #If "booksOrdered" is non-empty list (i.e. Books found that match search request criteria)
        ...
        altMessageModified = False                                          #Only return "altMessage" content, concatenated to "message", if "True" (i.e. "altMessage" modified and set "altMessageModified to "True")
            ...
            if int(bookOrdered[1][1]) < 5:                                  #If "bookOrdered", "orderRequest" quantity less than 5
                tempStr = (str(counter) + ") " +                            #Create "tempStr" (Temporary String) w/ numerical bulletpoint using "counter", concatenate
                           ...
                           str(float(bookOrdered[0][2]) * 
                               int(bookOrdered[1][1])) + '\n')              #Book total price is price from "bookOrdered" multiplied by quantity from "bookOrdered" 
